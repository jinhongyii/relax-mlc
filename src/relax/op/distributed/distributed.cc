...
TVM_REGISTER_GLOBAL("relax.op.distributed.redistribute").set_body_typed(redistribute);

StructInfo InferStructInfoRedistribute(const Call& call, const BlockBuilder& ctx) {
    ...
    ICHECK(sinfo);
    return distributed::DTensorStructInfo(sinfo->tensor_sinfo, attrs->device_mesh,
                                          attrs->placement);
}
