    ...
    for (const auto& access_pr: buffer_access_indices_){
      ...
      for(int i = 0; i < indices.size(); i++){
        ...
        for (const auto& another_access_pr: buffer_access_indices_){
          ...
          for (int j = 0; j< another_indices.size();j++){
...
void BuildAxisGraphCallTIR(const Var& output_var, const Call& call, const tir::PrimFunc& func,
  ...
  for(const auto& var_axis_group : tir_var_axis_group_list){
    ...
    if(output_idx == -1){
      for(int i =1;i<var_axis_group.size();i++){
        axis_group_graph->JoinAxis({tir_var_to_relax_var[var_axis_group[i].first].get(), var_axis_group[i].second}, {tir_var_to_relax_var[var_axis_group[0].first].get(), var_axis_group[0].second}, distributed::AxisGroupGraph::EdgeType::kSimbling);
      }
    } else {
      for(int i=0;i<var_axis_group.size();i++){
        if(i != output_idx){
          axis_group_graph->JoinAxis({tir_var_to_relax_var[var_axis_group[i].first].get(), var_axis_group[i].second}, {tir_var_to_relax_var[var_axis_group[output_idx].first].get(), var_axis_group[output_idx].second}, distributed::AxisGroupGraph::EdgeType::kDescend);
        }
      }
    }
  }
