...
std::tuple<int, int> kernelLaunchConfig(AllReduceStrategyType algo, AllReduceParams& param, size_t elts_per_thread)
{
    ...
    default: LOG(FATAL)<<("Algorithm not supported here.");
    }
...
void customAllReduceInternal(AllReduceParams& params, void* data, size_t elts, size_t size_per_elem,
    DLDataType dataType, AllReduceStrategyType strat, cudaStream_t stream)
{
...
void CustomAllReduce(DLTensor* send, DLTensor* barrier_in, DLTensor* barrier_out, DLTensor* recv) {
  ...
  params.barrier_flag = counter++;
  std::vector<void*> remote_ptrs = ipc_alloc->GetIPCRemoteMemPtr(send->data);
  for (int i = 0; i < ctx->worker->num_workers; ++i) {
    params.peer_comm_buffer_ptrs[i] = remote_ptrs[i];
  }
  remote_ptrs = ipc_alloc->GetIPCRemoteMemPtr(barrier_in->data);
  for (int i = 0; i < ctx->worker->num_workers; ++i){
      params.peer_barrier_ptrs_in[i] = reinterpret_cast<uint32_t*>(remote_ptrs[i]);
  }
  remote_ptrs = ipc_alloc->GetIPCRemoteMemPtr(barrier_out->data);
  for (int i = 0; i < ctx->worker->num_workers; ++i){
      params.peer_barrier_ptrs_out[i] = reinterpret_cast<uint32_t*>(remote_ptrs[i]);
  }

  customAllReduceInternal(params, recv->data, TensorSize(send), send->dtype.bits / 8, send->dtype, AllReduceStrategyType::ONESHOT, ctx->default_stream);
}

TVM_REGISTER_GLOBAL("cuda_ipc.custom_allreduce").set_body_typed(CustomAllReduce);
