...
from tvm.relax.op.distributed import redistribute as _redistribute, annotate_sharding as _annotate_sharding
...
def _lookup_device_mesh(device_mesh_str: py_str) -> DeviceMesh:
    ...
    name, index = device_mesh_str.split("[")
    index = int(index[:-1])
...
def annotate_sharding(value: Expr, device_mesh: Union[py_str, DeviceMesh], placement: Union[py_str, Placement]) -> Expr:
...
def redistribute(value: Expr, device_mesh: Union[py_str, DeviceMesh], placement: Union[py_str, Placement]) -> Expr:
    if isinstance(device_mesh, py_str):
        device_mesh = _lookup_device_mesh(device_mesh)
    if isinstance(placement, py_str):
        placement = Placement.from_text(placement)
